...
4. The DOE Schemathesis behavioural run against the live prototype (dummy token, the practice of the doe-iri validation matrix) passes: 48 operations passed, 0 failed, after the route-aware `Allow` header fix.
5. Both platform workflows exist and are verified: `iri-exec` carries the workflow executor's scripts to a cluster login node with the exit code intact, and `iri-job` completed a round trip from an ACTIVATE workflow through the public prototype endpoint to a Slurm job on the lab cluster and back.
